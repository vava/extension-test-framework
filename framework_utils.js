...
this.assert = function() {
	...
	that: function(actual, pred) {
		...
		if (nice_actual.indexOf("\n") != -1 && nice_expected.indexOf("\n") != -1) {
		    ...
		    log("Assert #" + number + ": < expected: \n" + nice_expected + "\n:>, < got: \n" + nice_actual + "\n:>, < difference between (-)expected and (+)got : \n" + diff + "\n:>");
		} else {
		    log("Assert #" + number + ": < expected: " + nice_expected + " :>, < got: " + nice_actual + " :>");
		}
...
let ppJson = function(el) {
	...
	if (el == null) {
		...
		for (let [,value] in Iterator(el)) {
		    let attr_value = new_indent + intPP("", value);
		...
		for (let [name, value] in Iterator(el)) {
		    ...
		    let attr_value = intPP(new_indent, value);
...
let make_diff = function(a, b) {
	...
	let d = o.next[1];
	while(d.prev.length != 2) {
	    string_diff += "-" + d.text + "\n";
	    d = d.next[0];
	}
	let a = o.next[0];
	while(a.prev.length != 2) {
	    string_diff += "+" + a.text + "\n";
	    a = a.next[0];
	}
	o = a;
